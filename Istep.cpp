...
Istep::Istep(void) :
  DefaultGUIModel("Istep", ::vars, ::num_vars), dt(
      RT::System::getInstance()->getPeriod() * 1e-6), period(1.0), delay(0.0),
      Amin(-20.0), Amax(20.0), Nsteps(2), Ncycles(1), duty(50), offset(0.0)
{
  QWhatsThis::add(
      this,
      "<p><b>I-Step:</b><br>This module generates a train of current injection pulses"
        "with amplitudes between a user-specified minimum and maximum.</p>");
...
void
Istep::update(DefaultGUIModel::update_flags_t flag)
{
  switch (flag)
    {
  case INIT:
